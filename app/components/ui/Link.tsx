type IconProps = {
  children?: React.ReactNode
  arrow: 'white' | 'green' | 'black'
  bg?: 'white' | 'green' | 'black';
}

const color = {
  black: 'var(--color-primary)',
  white: 'var(--color-white)',
  green: 'var(--color-accent)',
}


export default function Link({ arrow, bg, children='Label' }: IconProps) {
  return (
    <a href="#" className={`flex group gap-[15px] items-center ${bg ? '' : ''}`}>
      <svg className={`${bg ? '' : 'order-2'}`} width={`${bg ? 41 : 21}`} height={`${bg ? 41 : 20}`} viewBox={bg ? "0 0 41 41" : "10 10 21 20"} fill="none" xmlns="http://www.w3.org/2000/svg">
        {bg && <circle cx="20.5" cy="20.5" r="20.5" fill={`${color[bg]}`}/>}
        <path d="M11.2501 24.7009C10.5326 25.1151 10.2868 26.0325 10.701 26.75C11.1152 27.4674 12.0326 27.7132 12.7501 27.299L11.2501 24.7009ZM30.7694 16.3882C30.9839 15.588 30.509 14.7655 29.7088 14.5511L16.6688 11.057C15.8686 10.8426 15.0461 11.3175 14.8317 12.1177C14.6173 12.9179 15.0921 13.7404 15.8923 13.9548L27.4834 17.0606L24.3776 28.6517C24.1632 29.4519 24.6381 30.2744 25.4383 30.4888C26.2385 30.7033 27.061 30.2284 27.2754 29.4282L30.7694 16.3882ZM12.7501 27.299L30.0706 17.299L28.5706 14.7009L11.2501 24.7009L12.7501 27.299Z" fill={`${color[arrow]}`}/>
      </svg>
      <p className={`text-p link ${bg ? '' : 'order-1'}`} style={bg ? undefined : { color: color[arrow] }}>{children}</p>
    </a>
  )

}